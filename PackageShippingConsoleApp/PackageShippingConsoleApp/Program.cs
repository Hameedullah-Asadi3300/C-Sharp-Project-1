...
namespace PackageShippingConsoleApp
{
    internal class Program
    {
        static void Main()
        {
           ...
           if (weight > 50)    //This is branching which evaluates if the weight is more than packageWeight
            {
           ...
           //PACKAGE WIDTH
            ...
            int height = Convert.ToInt32(Console.ReadLine());   //Converts the user's input to string
            ...
            if (length > 50)      //This is branching which evaluates if  the length is more than packagelength
            {
                Console.WriteLine("We are sorry, the package is too big to be shipped via Package Express"); //It will print if the if is true
            }
            decimal quote = (weight * width * length) * weight / 100;
